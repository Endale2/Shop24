<template>
    <aside class="w-48 bg-gray-800 text-white p-4">
      <ul>
        <li>
          <router-link to="/dashboard" class="block py-2" active-class="font-bold">Dashboard</router-link>
        </li>
        <li>
          <router-link to="/products" class="block py-2" active-class="font-bold">Products</router-link>
        </li>
        <li>
          <router-link to="/stores" class="block py-2" active-class="font-bold">Stores</router-link>
        </li>
        <li>
          <router-link to="/customers" class="block py-2" active-class="font-bold">Customers</router-link>
        </li>
      </ul>
      
    </aside>
</template>