  ...
  <form @submit.prevent="submitForm" class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
      <div>
        ...
        <input v-model.number="form.price" type="number" min="0" step="0.01" required class="mt-1 w-32 border rounded px-3 py-2"/>
      </div>
      <div>
    ...
    <div>
      ...
      <div v-for="(img, idx) in form.images" :key="idx" class="flex items-center space-x-2 mb-2">
        <input v-model="form.images[idx]" type="url" placeholder="https://…" class="flex-1 border rounded px-3 py-2"/>
        <button type="button" @click="removeImage(idx)" class="text-red-600">
    ...
    <div>
      ...
      <div v-for="(v, vi) in form.variants" :key="vi" class="border rounded p-4 mb-4">
        ...
        <div v-for="(val, key) in v.options" :key="key" class="flex items-center space-x-2 mb-2">
          <input v-model="variantKeyTemp[vi][key]" @blur="renameOptionKey(vi, key, variantKeyTemp[vi][key])" placeholder="Option Name" class="w-1/3 border rounded px-2 py-1 text-sm"/>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
          <div>
            ...
            <input v-model.number="v.price" type="number" min="0" step="0.01" required class="mt-1 w-32 border rounded px-3 py-2"/>
          </div>
          <div>
      ...
      <button type="button" @click="addVariant" class="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">
    ...
    <div class="flex justify-end space-x-4 mt-8">
      ...
      <button type="submit" :disabled="saving" class="px-6 py-2 bg-green-600 text-white rounded hover:bg-green-700 disabled:opacity-50">
