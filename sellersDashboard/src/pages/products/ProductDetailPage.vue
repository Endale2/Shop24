<template>
  <div class="min-h-full bg-gray-50">
    <div class="p-4 sm:p-6 lg:p-8">
      ...
      <!-- Product Content -->
      <div v-else-if="product.id" class="space-y-6">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 md:p-8 space-y-6">
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            <!-- Product Images -->
            <div class="space-y-4">
              <div class="w-full h-64 sm:h-72 md:h-64 lg:h-72 rounded-lg overflow-hidden shadow-sm border border-gray-200 relative bg-gray-50 flex items-center justify-center">
                <img
                  v-if="mainImage"
                  :src="mainImage"
                ...
                <div v-else class="w-full h-full flex items-center justify-center bg-gray-100">
                  <PlaceholderIcon class="w-16 h-16 text-gray-400" />
                </div>
              </div>
              ...
              <div v-if="allImages.length > 1" class="flex space-x-3 overflow-x-auto pb-2">
                <img
                  ...
                  class="w-16 h-16 object-cover rounded-md cursor-pointer border-2 shadow-sm transform hover:scale-105 transition-all duration-200 ease-in-out"
                />
              </div>
            </div>
            ...
            <div class="lg:sticky lg:top-8 self-start space-y-4">
              <div>
                ...
                <div class="text-sm text-gray-600 mt-2">
                  <div v-if="product.collection_ids && product.collection_ids.length > 0" class="space-y-1">
                    <div v-for="collectionId in product.collection_ids" :key="collectionId" class="inline-block bg-gray-100 px-2 py-1 rounded-full text-xs mr-2 mb-1">
              ...
              <div class="space-y-3">
                <div class="flex items-baseline space-x-2">
                  <p class="text-3xl sm:text-4xl font-bold text-green-700">
                ...
                <div class="flex items-center space-x-4">
                  <div class="flex items-center space-x-2">
                    <span class="text-sm text-gray-700 font-medium">Stock:</span>
                    <span 
                      :class="{
              ...
              <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 mt-6">
                <button
                  ...
                  class="flex-1 w-full sm:w-auto inline-flex items-center justify-center px-4 py-2.5 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition duration-150 ease-in-out"
                >
                ...
                <button
                  ...
                  class="flex-1 w-full sm:w-auto inline-flex items-center justify-center px-4 py-2.5 border border-red-300 text-sm font-medium rounded-lg shadow-sm text-red-700 bg-white hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition duration-150 ease-in-out"
                >
        ...
        <!-- Variants Section -->
        <div v-if="product.variants && product.variants.length > 0" class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div class="px-4 py-3 border-b border-gray-100 bg-gray-50">
            <h2 class="text-sm font-semibold text-gray-900">Available Variants</h2>
          </div>
          <div class="p-4">
            <div class="space-y-3">
              <div
                ...
                :key="i"
                class="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center p-3 bg-green-50 rounded-lg shadow-sm border border-green-100 transition-all duration-200 ease-in-out hover:shadow-md hover:border-green-200"
              >
                ...
                <div class="text-right">
                  ...
                  <p class="text-xs font-medium" :class="{ 'text-green-600': v.stock > 0, 'text-red-600': v.stock <= 0 }">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div class="px-4 py-3 border-b border-gray-100 bg-gray-50">
            <h2 class="text-sm font-semibold text-gray-900">Product Information</h2>
          </div>
          <div class="p-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-500">
              <div class="space-y-2">
                <p><strong class="text-gray-700">Created:</strong> {{ formatDate(product.createdAt) }}</p>
                <p><strong class="text-gray-700">Last Updated:</strong> {{ formatDate(product.updatedAt) }}</p>
                <p><strong class="text-gray-700">Collections:</strong> 
              ...
              <div class="space-y-2">
                <p><strong class="text-gray-700">Product ID:</strong> {{ product.id }}</p>
                <p><strong class="text-gray-700">Shop ID:</strong> {{ product.shop_id }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-else class="text-center py-16 text-gray-400">
        <div class="max-w-md mx-auto">
          ...
          <p class="text-sm font-medium text-gray-600 mb-1">Product not found</p>
