...
func AdminLogin(c *gin.Context) {
	...
	// Authenticate and retrieve authData, adminData
	...
	// Set the access token as an HTTP-only cookie (expires in 15 minutes)
	c.SetCookie("access_token", authData.AccessToken, 15*60, "/", "", false, true)
	// Set the refresh token as an HTTP-only cookie (expires in 7 days)
	c.SetCookie("refresh_token", authData.RefreshToken, 7*24*3600, "/", "", false, true)
